...
/**************************************************************************************************\
|    EECS 281 Lab 3: String Library | | | |    During lab, you learned about C++
strings and how they work. In this coding exercise, you     | |    will be
implementing a select few functions from the C++ <string> library. If you do not
| |    understand everything in this file, do not worry! You will only need to
complete the          | |    functions marked with a TODO, and we will provide
you with all the information you need       | |    to complete this assignment.
To locate the portions of this file you need to complete,        | |    simply
find all 8 TODOs in this file (but feel free to explore the rest, if you wish).
| | |
\**************************************************************************************************/
...
String::String(const char *cstr_) : cstr{&a_null_byte}, sz{0}, allocation{0} {
...
// member variables to the empty state (do not initialize "this" String and
// swap).
...
// The move assignment operator simply swaps the contents of rhs without any
// copying.
...
// This assignment operator assigns the left-hand side with a copy of the String
// rhs. The implementation uses copy-swap.
...
// This assignment operator creates a temporary String object from the rhs
// C-string and swaps the contents using copy-swap.
...
// The subscript operator for String objects can be used to retrieve a reference
// to a character at index i of the string.
...
// This implementation is the same as above, but it works on const String
// objects.
...
// The c_str() function returns a pointer to the internal C-string object that
// stores the current contents of the String object.
...
// This function returns a reference to the first character of the String. You
// may assume that this function will NEVER be called on empty strings.
char &String::front() {
  return *cstr;
}

// back() for String Objects
// This function returns a reference to the last character of the String. You
// may assume that this function will NEVER be called on empty strings.
char &String::back() {
  return *(cstr + sz - 1);
}
...
// When substr() is called using two parameters, "pos" and "len", this function
// returns a newly constructed String object with its value initialized to a
// copy of a substring of the original String object, where this substring
// starts at the character position represented by "pos" and spans "len"
// characters, or until the end of the string, whichever comes first.
//
// This function uses the allocate_string(const char* str, int begin, int len)
// function which returns a char* object that spans a certain length starting
// from a given position. For example:
//     If char *myCstr represents a C-string with a value {'e', 'e', 'c', 's',
//     '2', '8', '1', '\0'}, calling char *newCstr = allocate_string(myCstr, 2,
//     3) would set newCstr to {'c', 's', '2', '\0'}.
// Then a String object is created by using the constructor defined on line 203.
// allocate_string() uses dynamic memory (calling new), so delete must be called
// on the C-string You may assume that the starting position "pos" < sz, and
// "len" will be at least 0.
//
// The "len" parameter is optional - if not defined, the "len" is set to a value
// of String::npos, and the substring is built to the very end of the String.
String String::substr(size_t pos, size_t len) const {
  // If the position exceeds the string's size, throw an out of bounds
  // exception.
  if (pos > sz) {
...
// "len" is not defined, the String is erased from "pos" to the end of the
// String. A reference to the modified String object is returned.
//
// You may assume that the starting position "pos" will be valid, and "len" will
// be >= 0.
//
// HINTS: Start with two pointers or indices, one that begins at the position to
// begin erasing, and one that begins at the character "len" distance away from
// "pos" (the first character after "pos" that won't be erased). Make sure to
// end the new string with the sentinel (null character), which can be assigned
// using the member variable "a_null_byte" (e.g. *ptr = a_null_byte). Don't
// forget to update the size of the string, "sz".
...
//
// HINTS: recall how an array works when you try to insert an element - all
// other elements after the insertion point must be shifted. This is true here
// as well. First, you must shift all characters after the insertion point by a
// certain distance. Then, you would insert the contents of the new String into
// the slots you have freed up for insertion. Make sure you add '\0' to the end
// of the string after inserting, and "sz" is updated.
...
//
// You may assume that the starting position "pos" will be valid, and "len" will
// be >= 0. If the value of "len" exceeds the end of the String object, replace
// as many characters as possible. A value of String::npos indicates that all
// characters to the end of the String should be replaced.
//
// HINT: You may make use other functions in this library to write a short
// solution.
...
// before position "pos" are ignored. It is enough for one single character of
// the sequence to match for the search to be successful. If a match is found,
// the function returns a size_t that represents the position of the first
// character that matches. Otherwise, the function returns npos. If "pos"
// exceeds the length of the string, the function will never find a match. If
// "pos" is not specified, the value of "pos" is assumed to be 0.
//
// HINTS: This is similar to the find() operation, but you will only need ONE
...
// toward 0). Characters after position "pos" are ignored. It is enough for one
// single character of the sequence to match for the search to be successful. If
// a match is found, the function returns a size_t that represents the position
// of the last character that matches. Otherwise, the function returns npos. If
// "pos" exceeds the length of the string, the entire String is searched.
//
...
String &String::operator+=(const String &rhs) {
  if (rhs.size() > 0) {
    ...
    for (char *dest = cstr + sz + rhs.sz, *source = rhs.cstr + rhs.sz;
         dest >= cstr + sz; --dest, --source)
      *dest = *source;
    sz += rhs.sz;
  }
  return *this;
...
// Helper for modifiers; call grow_allocation for the requested amount if need
// be.
...
// Allocates a C-string, constructed on the range from begin to end of C-string
// str.
...
If one of the arguments is a C-string, the String constructor will automatically
create a temporary String for it to match this function (inefficient, but
instructive). This automatic behavior would be disabled if the String
constructor was declared "explicit". This function constructs a copy of the lhs
in a local String variable, then concatenates the rhs to it with operator +=,
and returns it. */
