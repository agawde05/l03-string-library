...
find all the TODOs in this file (but feel free to explore the rest, if you
wish).      | | |
\**************************************************************************************************/

// This lab assignment was written by Jake Hage and Andrew Zhou for Winter 2019
// and simplified for Fall 2019. For instructions on how to implement this lab,
// please see the lab assignment.

// You should write your code in the "String.cpp" file (this is just the
// interface). You do NOT need to modify this file!
...
public:
  ...
  // If both i = size and len = 0, the input is valid and the result is an empty
  // string. Throw exception if the input is invalid.
  ...
  // It either directly inserts the new data into this String's space if it is
  // big enough, or allocates new space and copies in the old data with the new
  // data inserted. This String retains the final allocation. The behavior of
  // inserting a String into itself is not specified.
  ...
  // These functions all find a particular string or character(s) within this
  // String object. */
  ...
  // They do not create any temporary String objects. They either directly copy
  // the rhs data into the lhs space if it is big enough to hold the rhs, or
  // allocate new space and copy the old lhs data into it followed by the rhs
  // data. The lhs object retains the final memory allocation. If the rhs is a
  // null byte or an empty C-string or String, no change is made to lhs String.
...
private:
  ...
  static bool
      messages_wanted; // whether to output constructor/destructor/operator=
                       // messages, initially false

  // Helper for modifiers; call grow_allocation() for the requested amount if
  // need be.
  ...
  // Allocates a C-string, constructed on the range from begin to end of
  // C-string str.
